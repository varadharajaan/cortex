...
| S-14 | WireMock HTTPS for local smoke | HTTP/1.1 plaintext on host 8081 | HTTPS on host 8443 with a step-ca-issued cert; mappings unchanged | P5.1 (pairs with S-13) | Even stub upstreams should be encrypted in local-smoke so the gateway exercises its TLS path. Ollama itself still HTTP/1.1 plaintext in prod (R1.a exception per T-11) -- prod adds a TLS-terminating sidecar. |
