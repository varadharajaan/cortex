...
# ---------------------------------------------------------------------------
# CORTEX :: log-gateway runtime image (P10.0 / ADR-0050).
#
...
#   docker build -f infra/docker/log-gateway.Dockerfile -t cortex/log-gateway:0.1.0 .
# ---------------------------------------------------------------------------
...
RUN --mount=type=cache,target=/root/.m2 \
    ...
    ./mvnw -q -B -ntp -pl log-gateway -am -Dmaven.test.skip=true \
    ...
    ./mvnw -q -B -ntp -pl log-gateway -Dmaven.test.skip=true \
        -Dcheckstyle.skip=true -Dspotbugs.skip=true \
        -Ddependency-check.skip=true -Dcyclonedx.skip=true \
...
COPY --from=builder /build/log-gateway/target/log-gateway-0.1.0-SNAPSHOT.jar /app/app.jar
