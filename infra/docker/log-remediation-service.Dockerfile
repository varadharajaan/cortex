...
# ---------------------------------------------------------------------------
# CORTEX :: log-remediation-service runtime image (P10.0 / ADR-0050).
#
...
#   docker build -f infra/docker/log-remediation-service.Dockerfile -t cortex/log-remediation-service:0.1.0 .
# ---------------------------------------------------------------------------
...
RUN --mount=type=cache,target=/root/.m2 \
    ...
    ./mvnw -q -B -ntp -pl log-remediation-service -am -Dmaven.test.skip=true \
    ...
    ./mvnw -q -B -ntp -pl log-remediation-service -Dmaven.test.skip=true \
        -Dcheckstyle.skip=true -Dspotbugs.skip=true \
        -Ddependency-check.skip=true -Dcyclonedx.skip=true \
...
COPY --from=builder /build/log-remediation-service/target/log-remediation-service-0.1.0-SNAPSHOT.jar /app/app.jar
RUN chown -R cortex:cortex /app
USER cortex
EXPOSE 8096
ENV JAVA_OPTS="-XX:MaxRAMPercentage=75.0"
HEALTHCHECK --interval=15s --timeout=5s --start-period=60s --retries=10 \
  CMD curl -fsS http://localhost:8096/actuator/health || exit 1
