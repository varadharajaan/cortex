...
# ---------------------------------------------------------------------------
# CORTEX :: log-ingest-service runtime image (P10.0 / ADR-0050).
#
...
#   docker build -f infra/docker/log-ingest-service.Dockerfile -t cortex/log-ingest-service:0.1.0 .
# ---------------------------------------------------------------------------
...
RUN --mount=type=cache,target=/root/.m2 \
    ...
    ./mvnw -q -B -ntp -pl log-ingest-service -am -Dmaven.test.skip=true \
    ...
    ./mvnw -q -B -ntp -pl log-ingest-service -Dmaven.test.skip=true \
        -Dcheckstyle.skip=true -Dspotbugs.skip=true \
        -Ddependency-check.skip=true -Dcyclonedx.skip=true \
...
COPY --from=builder /build/log-ingest-service/target/log-ingest-service-0.1.0-SNAPSHOT.jar /app/app.jar
RUN chown -R cortex:cortex /app
USER cortex
EXPOSE 8092
ENV JAVA_OPTS="-XX:MaxRAMPercentage=75.0"
HEALTHCHECK --interval=15s --timeout=5s --start-period=60s --retries=10 \
  CMD curl -fsS http://localhost:8092/actuator/health || exit 1
