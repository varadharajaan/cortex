...
# ---------------------------------------------------------------------------
# CORTEX :: log-echo-service runtime image (P10.0 / ADR-0050).
#
...
#   docker build -f infra/docker/log-echo-service.Dockerfile -t cortex/log-echo-service:0.1.0 .
# ---------------------------------------------------------------------------
...
RUN --mount=type=cache,target=/root/.m2 \
    ...
    ./mvnw -q -B -ntp -pl log-echo-service -am -Dmaven.test.skip=true \
    ...
    ./mvnw -q -B -ntp -pl log-echo-service -Dmaven.test.skip=true \
        -Dcheckstyle.skip=true -Dspotbugs.skip=true \
        -Ddependency-check.skip=true -Dcyclonedx.skip=true \
...
COPY --from=builder /build/log-echo-service/target/log-echo-service-0.1.0-SNAPSHOT.jar /app/app.jar
