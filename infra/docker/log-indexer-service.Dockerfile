...
# ---------------------------------------------------------------------------
# CORTEX :: log-indexer-service runtime image (P10.0 / ADR-0050).
#
...
#   docker build -f infra/docker/log-indexer-service.Dockerfile -t cortex/log-indexer-service:0.1.0 .
# ---------------------------------------------------------------------------
...
RUN --mount=type=cache,target=/root/.m2 \
    ...
    ./mvnw -q -B -ntp -pl log-indexer-service -am -Dmaven.test.skip=true \
    ...
    ./mvnw -q -B -ntp -pl log-indexer-service -Dmaven.test.skip=true \
        -Dcheckstyle.skip=true -Dspotbugs.skip=true \
        -Ddependency-check.skip=true -Dcyclonedx.skip=true \
...
COPY --from=builder /build/log-indexer-service/target/log-indexer-service-0.1.0-SNAPSHOT.jar /app/app.jar
RUN chown -R cortex:cortex /app
USER cortex
EXPOSE 8097
ENV JAVA_OPTS="-XX:MaxRAMPercentage=75.0"
HEALTHCHECK --interval=15s --timeout=5s --start-period=60s --retries=10 \
  CMD curl -fsS http://localhost:8097/actuator/health || exit 1
