...
| `log-remediation.postman_collection.json`         | Actuator (`health,info,metrics,prometheus,beans`) + P9.3a `GET /api/v1/anomalies?tenantId&limit=10` read API + validation error + `cortex_remediation_dispatched_total{channel,outcome,tenant_id}` exposition (P6.0a OCP-bootstrap-registered, ADR-0036) + WireMock-shaped channel POSTs (`/services/SMK-postman/B/X` slack, `/v2/enqueue` pagerduty, `/rest/api/3/issue` jira) that mirror `scripts/smoke-p6-1a.ps1` LD23. The service is a Kafka consumer of `cortex.anomalies.v1` and now owns the anomalies read model. P6.1a + P9.3a closer. |
| `log-remediation.postman_environment_local.json`  | Local dev (`http://localhost:8096`; smoke stack via `scripts/live-e2e/smoke-p9-3a.ps1 -KeepInfra -KeepService` for seeded anomaly API assertions, or `scripts/smoke-p6-1a.ps1 -Provider all` for channel-adapter assertions). |
...
| `log-indexer.postman_collection.json`             | Actuator (`health,info,metrics,prometheus,beans`) + `health/quickwit` indicator (binder-gate proof) + `cortex_indexer_index_admin_total{backend,outcome,tenant_id}` + `cortex_indexer_search_total{backend,outcome,tenant_id}` exposition (P7.0..P7.4 OCP-bootstrap-registered, LD106 + LD112) + direct Quickwit reachability probes (`/health/livez`, `/api/v1/indexes`) + Quickwit search 404 permanent contract probe. NO business REST surface in P7.1a -- the SPI is invoked by the future P8 read-path service; the cross-phase Failsafe IT (`QuickwitCrossPhaseIT`) is the source of truth for the autowired admin + search beans. P7.1a closer. |
...
### log-processor collection (P5.4, 13 requests / 55 assertions)
...
| Admin (actuator)  | GET `{{base_url}}/actuator/prometheus`                                                    | 200 + exposition contains `cortex_processor_events_{consumed,parsed,classified}_total` (counter registration) + conditional `cortex_processor_sink_{loki,quickwit}_published_total` presence checks when sinks are enabled (P5.3 / ADR-0030) + unconditional `cortex_processor_anomalies_published_total` presence (P5.4 / ADR-0031, bootstrap-registered). |
| Metrics-Baseline  | GET `{{base_url}}/actuator/prometheus` (snapshot)                                         | 200 + capture `consumed_total` sum + `classified_total{outcome=anomaly}` + `sink_loki_published_baseline` + `sink_quickwit_published_baseline` + `anomalies_published_baseline` into env vars for the delta assertion (P5.3 sink baselines are zero when the sinks are disabled, which is fine; P5.4 anomalies counter is bootstrap-registered so the baseline is always present).        |
...
Folder order matters: `Auth` populates `jwt`, `Metrics-Baseline` snapshots the counters (including the P5.3 sink baselines + the P5.4 anomalies-published baseline), `Pipeline` publishes through the gateway and then waits 20 seconds for the outbox poller + Kafka consumer + anomalies publisher to drain, and `Metrics-After` reads `/actuator/prometheus` on the processor and asserts the deltas (event-pipeline + anomalies-published counters always; sink counters only when the family is present in the scrape, so the same collection passes against a stack with sinks disabled and against a stack with `cortex.processor.sinks.{loki,quickwit}.enabled=true`). Run with `--bail`. The pipeline assertion REQUIRES the full stack booted via `scripts/p5-4/boot-full-stack.ps1` so that the processor is started with `CORTEX_PROCESSOR_CLASSIFIER=spring-ai` + `SPRING_AI_OLLAMA_BASE_URL=http://localhost:8094` + `CORTEX_PROCESSOR_SINKS_LOKI_ENABLED=true` + `CORTEX_PROCESSOR_SINKS_QUICKWIT_ENABLED=true` and WireMock on `:8094` returns a deterministic anomaly verdict + the Loki / Quickwit stub mappings under `infra/local/wiremock/mappings/{loki-push,quickwit-ingest}.json` (ADR-0030 D6) return 204 / 200 + the broker has the `cortex.anomalies.v1` topic pre-created (ADR-0031). Newman invocation: `npx newman run postman\log-processor.postman_collection.json -e postman\log-processor.postman_environment_local.json --reporters cli --bail`.

Mirror in `scripts/p5-4/smoke-p5-4.ps1` (LD23 / LD101 closed-loop rule): every HTTP-observable assertion in the collection above has a matching assertion in the smoke script (and vice versa). Items 1-7 mirror smoke-p5-4 steps 01-07; items 8-11 mirror steps 08-11 plus the sink baseline + delta legs (loki + quickwit `published_total` strictly increases by 1 after the pipeline leg, asserted against the same WireMock journal that the smoke script checks) plus the P5.4 anomalies-published unconditional delta + a one-shot Kafka topic read on `cortex.anomalies.v1` that asserts a CloudEvent envelope with the documented two headers + record key + `data` fields landed (ADR-0031); items 12-13 are negative-path probes that the smoke script also performs.

Just import and run: open Postman, import `log-processor.postman_collection.json` and `log-processor.postman_environment_local.json`, pick the `log-processor-service :: local` environment, boot the full stack with `scripts\p5-4\boot-full-stack.ps1`, and hit "Run collection".
...
### log-remediation collection (P6.1a + P9.3a, 13 requests / 52 seeded-local assertions)
...
| Anomalies Read API  | GET `{{base_url}}/api/v1/anomalies?tenantId={{tenant_id}}&limit=10`                       | 200 + JSON array + limit respected + stable anomaly row fields. When `anomaly_event_id` is set by `scripts/live-e2e/smoke-p9-3a.ps1`, asserts that exact Kafka-seeded event is returned from the Postgres read model. |
| Anomalies Read API  | GET `{{base_url}}/api/v1/anomalies?limit=10`                                              | 400 + `errorCode=VALIDATION_FAILED` because `tenantId` is required until gateway P9.3b forwards authenticated tenant context. |
...
Folder order matters: `Admin (actuator)` first proves the surface is reachable + the counter family is registered (this gates everything downstream); `Anomalies Read API` verifies the P9.3a tenant-scoped query contract; `Metrics-Baseline` snapshots the per-channel sums; `Channel-Mock-Smoke` exercises the on-the-wire payload contract against the local WireMock (offline-safe, no real Slack / PagerDuty / Jira calls); `Metrics-After` verifies the counter family is still exposed and non-decreasing. Run with `--bail`. For the P9.3a seeded local gate, boot the smoke stack first via `scripts/live-e2e/smoke-p9-3a.ps1 -KeepInfra -KeepService` and copy the printed `tenant_id` + `anomaly_event_id` into the Newman command:
...
| Admin (actuator)     | GET `{{base_url}}/actuator/health/liveness`                                                | 200 + `status: UP` (K8s probe).                                                                                      |
| Admin (actuator)     | GET `{{base_url}}/actuator/health/readiness`                                               | 200 + `status: UP` (K8s probe).                                                                                      |
| Admin (actuator)     | GET `{{base_url}}/actuator/health/quickwit`                                                | 200 + `status: UP` + `details.backend == quickwit` (binder-gate proof for `cortex.indexer.admin.backend` + `cortex.indexer.search.backend`; P7.1a / ADR-0043 D1). |
| Admin (actuator)     | GET `{{base_url}}/actuator/info`                                                           | 200 + JSON object.                                                                                                   |
| Admin (actuator)     | GET `{{base_url}}/actuator/metrics`                                                        | 200 + `names` includes BOTH `cortex.indexer.index_admin_total` (LD106 admin bootstrap) AND `cortex.indexer.search_total` (LD112 search bootstrap). |
| Admin (actuator)     | GET `{{base_url}}/actuator/prometheus`                                                     | 200 + exposition contains BOTH counter families with `# HELP` + `# TYPE counter` comments + the allowlisted `backend` / `outcome` / `tenant_id` tags (Part 17) + `backend="quickwit"` series present on both families (binder gate proven on the scrape). |
| Metrics-Baseline     | GET `{{base_url}}/actuator/prometheus` (snapshot)                                          | 200 + capture per-family sums (`cortex_indexer_index_admin_total{backend="quickwit"}` + `cortex_indexer_search_total{backend="quickwit"}`) into `index_admin_baseline_quickwit` + `search_baseline_quickwit` env vars for the Metrics-After non-decreasing check. |
| Quickwit-Admin       | GET `{{base_url}}/actuator/health/quickwit` (re-check after baseline)                      | 200 + `status: UP`.                                                                                                  |
| Quickwit-Admin       | GET `{{quickwit_base_url}}/health/livez`                                                   | 200 (direct Quickwit reachability proof; skipped when `quickwit_base_url` is empty -- staging / prod).               |
| Quickwit-Admin       | GET `{{quickwit_base_url}}/api/v1/indexes`                                                 | 200 + JSON array (direct Quickwit cluster-admin proof; skipped when `quickwit_base_url` is empty).                   |
| Quickwit-Search      | POST `{{quickwit_base_url}}/api/v1/cortex-no-such-index-v1/search` body `{"query":"*","max_hits":1}` | 404 (Quickwit confirms the wire-shape that `QuickwitHttpSearch` maps to `permanent_failure / quickwit:4xx:404`; ADR-0042 D5 "404 is permanent, NOT idempotent-success"; skipped when `quickwit_base_url` is empty). |
| Metrics-After        | GET `{{base_url}}/actuator/prometheus` (non-decreasing check)                              | 200 + both counter families still exposed with HELP+TYPE + per-family sums monotonically non-decreasing vs Metrics-Baseline. NOT a strict delta -- Postman has no inbound SPI endpoint to invoke from log-indexer-service in P7.1a (the SPI is invoked by the future P8 read-path service); the cross-phase Failsafe IT (`QuickwitCrossPhaseIT`) is the source of truth for autowired admin+search bean behaviour + counter deltas. |

Folder order matters: `Admin (actuator)` first proves the surface is reachable + BOTH counter families are registered + the `/health/quickwit` indicator is wired (this gates everything downstream); `Metrics-Baseline` snapshots both per-family sums; `Quickwit-Admin` re-checks the indicator and additionally hits Quickwit directly to prove the underlying datastore is reachable; `Quickwit-Search` asserts the 404-is-permanent wire contract that the `QuickwitHttpSearch` adapter relies on (ADR-0042 D5); `Metrics-After` verifies both counter families are still exposed and non-decreasing. Run with `--bail`. Boot the smoke stack first via `scripts/smoke-p7-1a.ps1 -KeepInfra` (which starts Quickwit 0.8.1 from `infra/local/docker-compose.smoke.yml` and runs the log-indexer-service on `:8097` with both binder gates flipped to `quickwit`) THEN run Newman against the still-running stack:
...
The collection has NO inbound-SPI-trigger requests by design -- log-indexer-service in P7.1a does NOT expose a REST controller for `ensureIndex` / `dropIndex` / `applyRetention` / `search` (the SPI is invoked by `IndexerService` from the inbound Kafka pipeline, which P7.1a does not exercise). The HTTP-observable surface for Postman / Newman is therefore: actuator health (the `/health/quickwit` indicator IS the proof that both binder gates are flipped from `noop` to `quickwit` and that `QuickwitHttpAdmin` + `QuickwitHttpSearch` beans were wired without `BeanCurrentlyInCreationException` -- see ADR-0043 D2 for the `@Lazy` cycle-break that surfaced from `QuickwitCrossPhaseIT`), the Prometheus exposition (the bootstrap loop in `IndexerMetrics.@PostConstruct` is proven by every failable outcome series being present even with zero ticks), and direct Quickwit reachability probes (because the upstream datastore being reachable is a precondition for any subsequent SPI call). For the autowired admin+search bean behaviour + counter deltas, the cross-phase Failsafe IT `QuickwitCrossPhaseIT` is the source of truth (Leg D); the Newman collection is the Leg C "HTTP contract + scrape exposition" mirror.

Mirror in `scripts/smoke-p7-1a.ps1` (LD23 / LD104 closed-loop rule): every HTTP-observable assertion in the collection above has a matching assertion in the smoke script (and vice versa). The actuator probes (health, liveness, readiness, `/health/quickwit`, metrics, prometheus) mirror the smoke health gate + counter exposition checks; the direct Quickwit probes (`/health/livez`, `/api/v1/indexes`) mirror the smoke's `Wait-Url` warm-up gates against the docker container; the 404-permanent search probe mirrors the smoke's negative path against the same Quickwit instance. The smoke-only assertions (`docker compose up quickwit` lifecycle + JVM PID cleanup via `Register-EngineEvent` exit trap + per-run transcript at `scripts/logs/p7-1a/`) are intentionally not in Newman because they are not HTTP-observable from the Postman boundary.
