 ...
 * the upstream log-gateway; if absent (direct smoke-test hit), a UUID
 * is generated. The value is echoed back as the same response header
 * so callers can correlate across hops.</p>
 *
 * <p>Runs as the highest-precedence filter so every downstream
 * component (controller, error handler) sees the id in MDC.</p>
 */
@Component
